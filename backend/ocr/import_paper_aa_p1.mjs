// Session 1 importer — Math AA HL Paper 1.
// Reads backend/data/paper_aa_p1_manifest.json and upserts each question via
...
const MANIFEST = path.join(ROOT, 'backend/data/paper_aa_p1_manifest.json');
...
const cat = db.prepare("SELECT COUNT(*) AS c FROM questions WHERE category='past' AND source LIKE 'AA HL P1%'").get().c;
const nw = db.prepare("SELECT COUNT(*) AS c FROM questions WHERE review_status='new' AND source LIKE 'AA HL P1%'").get().c;
console.log(`DB check -> AA HL P1 past rows: ${cat}, review_status='new': ${nw}`);
