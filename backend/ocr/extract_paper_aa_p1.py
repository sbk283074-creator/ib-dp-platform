#!/usr/bin/env python3
"""
Session 1 extractor — Math AA HL Paper 1 (past papers, 2021 May -> 2024.11).
Text-layer extraction (pypdfium2). Produces a JSON manifest of question records
(question text + rendered question_image, answer text + rendered answer_image) and
writes the JPGs to backend/public/figures/paper_aa_hl_p1/<slug>/.
...
MANIFEST = os.path.join(ROOT, "backend/data/paper_aa_p1_manifest.json")
...
PAPERS = [
 ("IB 数学 HL 真题（2006-23）/2021 May/IB 数学 AA  HL  2021.05/Mathematics_analysis_and_approaches_paper_1__TZ1_HL.pdf", "2021May_TZ1", "2021 May TZ1"),
 ("IB 数学 HL 真题（2006-23）/2021 May/IB 数学 AA  HL  2021.05/Mathematics_analysis_and_approaches_paper_1__TZ2_HL.pdf", "2021May_TZ2", "2021 May TZ2"),
 ("IB 数学 HL 真题（2006-23）/2021 Nov/Mathematics_analysis_and_approaches_paper_1__HL.pdf", "2021Nov", "2021 Nov"),
 ("IB 数学 HL 真题（2006-23）/2022 May/Mathematics_analysis_and_approaches_paper_1__TZ1_HL.pdf", "2022May_TZ1", "2022 May TZ1"),
 ("IB 数学 HL 真题（2006-23）/2022 May/Mathematics_analysis_and_approaches_paper_1__TZ2_HL.pdf", "2022May_TZ2", "2022 May TZ2"),
 ("IB 数学 HL 真题（2006-23）/2022 Nov/Mathematics_analysis_and_approaches_paper_1__HL.pdf", "2022Nov", "2022 Nov"),
 ("IB 数学 HL 真题（2006-23）/2023 May/Mathematics_analysis_and_approaches_paper_1__TZ1_HL.pdf", "2023May_TZ1", "2023 May TZ1"),
 ("IB 数学 HL 真题（2006-23）/2023 May/Mathematics_analysis_and_approaches_paper_1__TZ2_HL.pdf", "2023May_TZ2", "2023 May TZ2"),
 ("IB 数学 HL 真题（2006-23）/2023 Nov/Mathematics_analysis_and_approaches_paper_1__TZ1_HL.pdf", "2023Nov_TZ1", "2023 Nov TZ1"),
 ("IB 数学 HL 真题（2006-23）/2023 Nov/Mathematics_analysis_and_approaches_paper_1__TZ2_HL.pdf", "2023Nov_TZ2", "2023 Nov TZ2"),
 ("2024.5HL/Mathematics_analysis_and_approaches_paper_1__TZ1_HL.pdf", "2024_5_TZ1", "2024 May TZ1"),
 ("2024.5HL/Mathematics_analysis_and_approaches_paper_1__TZ2_HL.pdf", "2024_5_TZ2", "2024 May TZ2"),
 ("2024.11HL/Mathematics_analysis_and_approaches_paper_1__HL.pdf", "2024_11", "2024 Nov"),
]

qhead_re = re.compile(r'(?m)^\s*(\d+)\.\s*\[Maximum mark: (\d+)\]')
# MS question-boundary detector. Matches THREE formats seen across the IB
# corpus (all anchored at line start, strictly ordered 1..N by the walker):
#   1. "Question 10" / "Question 10 continued"  -> g1
#   2. "10." (some papers) — REQUIRES trailing whitespace (NOT \b, which is
#      false after "N." + space/newline) and must not be followed by a digit,
#      so decimals like "2.5"/"1.000." never false-match -> g2
#   3. "2 METHOD 1" (number + space + METHOD, NO dot) seen in e.g.
#      2021May_TZ1 -> g3
# Crucially excludes right-aligned math like "E(Y) = 11/4" — pypdfium2
# emits it on its own line, but there is no dot (and no " METHOD") after
# the number, so it is never taken as a question start (Bug 2026-08-25:
# the old bare \d+\b matched "11" in "11/4" as Q11's start, shifting every
# later span — Q10's continuation page was stolen by Q11).
...
# IB math PDFs encode math symbols in the Private Use Area (U+E000-U+F8FF)
# via the Symbol and MT Extra fonts. This translates the PUA text layer to
# Unicode so the question/answer text is readable and copy-pasteable.
# Structural glyphs (extensible brackets, fraction bars) are best-effort
# linearized; the rendered image remains the source of truth for display.
_FRAC_PAIR = re.compile(r'[\uf0e6-\uf0f2][\uf0f6-\uf0fc]')  # fraction bar (open+close piece)
_DELIM_PAIR = re.compile(  # adjacent extensible-delimiter pieces (short brackets)
    r'([\uf8eb\uf8ec\uf8ed\uf8ee\uf8ef\uf8f0\uf8f1\uf8f3])'
    r'([\uf8f6\uf8f7\uf8f8\uf8f9\uf8fa\uf8fb\uf8f2\uf8f4])'
)
_DELIM_ADJ = {
...
PUA_MAP = {
    # Symbol font: Greek uppercase (A..Z at 0x41..0x5A)
    ...
    # Symbol font extended: sets, operators, big ops (0xA0..)
    ...
    # Structural fraction pieces (singletons whose pair was consumed by _FRAC_PAIR)
    ...
    # Extensible delimiter pieces (fallback for separated tall brackets)
...
def normalize_math(text):
    """Translate Symbol/MT Extra PUA math glyphs to Unicode (best-effort)."""
    if not text:
        return text
    def _adj(m):
...
def extract_command(text):
    m = re.match(r'^\s*\d+\.\s*\[Maximum mark: \d+\]\s*\n?(.*)', text.strip(), re.S)
...
def process(qp_rel, slug, pretty):
    ...
    for idx, n in enumerate(keys):
        ...
        if idx+1 < len(keys):
            qpe = max(qps, page_of(qend, qoff) - 1)   # exclude next question's header page
        else:
            qpe = page_of(qend - 1, qoff)              # last question -> actual last page
        ...
        records.append({
            ...
            "source": f"AA HL P1 · {pretty}", "tags": [], "authored_by": "ib",
