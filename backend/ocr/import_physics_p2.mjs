// Session 5 importer — Physics HL Paper 2.
// Reads physics_hl_p2_manifest.json and performs idempotent DELETE+INSERT per source.
...
const MANIFEST = path.join(ROOT, 'backend/data/physics_hl_p2_manifest.json');
...
const count = db.prepare("SELECT COUNT(*) AS c FROM questions WHERE id LIKE 'PHYS_HL_P2_%'").get().c;
const fresh = db.prepare("SELECT COUNT(*) AS c FROM questions WHERE id LIKE 'PHYS_HL_P2_%' AND review_status='new'").get().c;
console.log(`\nTOTAL inserted: ${total}`);
console.log(`DB check -> Physics HL P2 rows: ${count}, review_status='new': ${fresh}`);
