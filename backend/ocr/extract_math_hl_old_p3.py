#!/usr/bin/env python3
"""
Session 14 extractor - Math HL Paper 3, FULL HISTORY (2008-2020, old "Math HL" guide).
Text-layer extraction (pypdfium2). Mirrors extract_math_hl_old_p1.py / _p2.py: derives
the question COUNT from the markscheme, walks the question paper by numbered questions
(1..N), pairing Q and A by question number. Same strict guard (rule #8: skip any
unreliable/old-format year, never ship it broken). Idempotent: stable ids; companion
Node importer does DELETE+INSERT per paper.
"""
...
MANIFEST = os.path.join(ROOT, "backend/data/paper_aa_p3_old_manifest.json")
    ...
    for ydir in sorted(os.listdir(BASE)):
        ...
        for dp,dn,fn in os.walk(root):
            for f in fn:
                ...
                if 'paper_3' not in f or 'HL' not in f or 'markscheme' in f.lower(): continue
...
def process(qp, ms, slug, pretty):
    ...
    if not ms or N<3 or N>25 or len(qstarts)!=N or len(mstarts)!=N:
    ...
    for idx,n in enumerate(keys):
        ...
        records.append({
            ...
            "source":f"HL P3 · {pretty}","tags":[],"authored_by":"ib",
...
def main():
    ...
    print(f"\nTOTAL questions (2008-2020 P3): {len(all_recs)}")
