// Session 14 importer - Math HL Paper 3, full history (2008-2020 old guide).
// Reads backend/data/paper_aa_p3_old_manifest.json and upserts each question via
...
const MANIFEST = path.join(ROOT, 'backend/data/paper_aa_p3_old_manifest.json');
...
console.log(`\nTOTAL inserted (2008-2020 P3): ${total}`);
const cat = db.prepare("SELECT COUNT(*) AS c FROM questions WHERE category='past' AND source LIKE 'HL P3%'").get().c;
const nw = db.prepare("SELECT COUNT(*) AS c FROM questions WHERE review_status='new' AND source LIKE 'HL P3%'").get().c;
console.log(`DB check -> HL P3 past rows: ${cat}, review_status='new': ${nw}`);
