...
// Map of book id -> absolute source-PDF path (generated by ocr/book_registry.py).
const __dirname = path.dirname(fileURLToPath(import.meta.url));
let BOOK_FILES = {};
try {
  BOOK_FILES = JSON.parse(fs.readFileSync(path.join(__dirname, 'book_files.json'), 'utf8'));
} catch { BOOK_FILES = {}; }
...
// GET /api/books/:id/file — stream the ORIGINAL book PDF so the UI can show
// the whole book (not just the extracted questions). Supports Range requests
// via res.sendFile, which matters for the multi-hundred-MB textbooks.
router.get('/books/:id/file', (req, res) => {
  const entry = BOOK_FILES[req.params.id];
  if (!entry || !entry.path) return res.status(404).json({ error: 'no source file for this book' });
  if (!fs.existsSync(entry.path)) return res.status(410).json({ error: 'source file missing on disk' });
  res.sendFile(entry.path, {
    headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'inline' }
  });
});

// GET /api/books/:id/availability — whether the full book PDF is readable here.
router.get('/books/:id/availability', (req, res) => {
  const entry = BOOK_FILES[req.params.id];
  res.json({ id: req.params.id, has_file: Boolean(entry && fs.existsSync(entry.path)) });
});
