...
import { askAI, aiStatus, checkRateLimit, AIError } from './ai.js';
...
// ---------------------------------------------------------------------------
// AI study assistant (Groq free tier — server-side key only, see src/ai.js).
// Safety: no key -> { configured:false }, never any cost; hard rate-limited;
// never an open proxy (client sends questionId + message, we ground the prompt
// in our own DB and never log prompt text).
// ---------------------------------------------------------------------------

// GET /api/ask/status -> is the server configured for AI? (key is NEVER returned)
router.get('/ask/status', (req, res) => {
  res.json(aiStatus());
});

// POST /api/ask -> answer a student's question
router.post('/ask', asyncHandler(async (req, res) => {
  const { message, questionId, subject, topic, marks, model, complexity, length } = req.body || {};

  // Server-side rate limit (soft backstop before Groq's hard cap).
  ...
  if (!rl.ok) {
    return res.status(429).json({
      ok: false,
      error: 'rate_limited',
      message: 'Too many requests. Please slow down.' + (rl.retryAfter ? ` Retry in ${rl.retryAfter}s.` : ''),
      retryAfter: rl.retryAfter || null
    });
  }

  // Ground the answer in OUR database instead of trusting the client to send text.
  let questionContext = '';
  ...
  if (questionId) {
    try {
      const row = await db.prepare(
        `SELECT subject, topic, paper_type, marks, question, answer, explanation FROM questions WHERE id = ?`
      ).get(questionId);
      if (row) {
        qSubject = qSubject || row.subject;
        qTopic = qTopic || row.topic;
        qMarks = qMarks ?? row.marks;
        questionContext = [
          `Q: ${row.question || ''}`,
          row.answer ? `A: ${row.answer}` : '',
          row.explanation ? `Explanation: ${row.explanation}` : ''
        ].filter(Boolean).join('\n');
      }
    } catch {
      // ignore — fall back to free-form message only
    }
  }

  try {
    const result = await askAI({ message, questionContext, subject: qSubject, topic: qTopic, marks: qMarks, model, complexity, length });
    if (result.error === 'quota_exceeded') {
      return res.status(429).json({ ok: false, error: 'quota_exceeded', message: result.answer, quota: result.quota || null });
    }
    res.json({
      ok: true,
    ...
    if (e instanceof AIError) {
      return res.status(e.status || 400).json({ ok: false, error: e.code, message: e.message, retryAfter: e.retryAfter || null });
    }
    throw e; // unexpected -> router error handler
  }
}));
